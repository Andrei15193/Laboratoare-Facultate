...
% - functia de aproximat este reprezentata prin coeficientii c ai seriei MacLauren trunchiate: 
...
fAproxNumarator = 0;
for i=1:m+1
	fAproxNumarator = fAproxNumarator + a(i)*x.^(i-1);
end

fAproxNumitor = 0;
for j=1:k+1
	fAproxNumitor = fAproxNumitor + b(j)*x.^(j-1);
end
