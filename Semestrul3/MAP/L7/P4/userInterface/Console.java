...
public class Console{
    ...
    public Console(controller.ApplicationController controller){
        this.input = new java.io.BufferedReader(new java.io.InputStreamReader(System.in));
    ...
    public void run(){
        try{
            ...
            do{
                ...
                if (userInterface.FirmMenuOptions.Add_Firm.ordinal() == option)
                    this.addFirm();
                else if (userInterface.FirmMenuOptions.Classify_Firms.ordinal() == option)
                    this.classifyFirms();
            }while (option != userInterface.FirmMenuOptions.Exit.ordinal());
            this.output.println("Application is closing");
        }
        catch (Exception e){
            this.printError("An unhandled exception occurred! " + e.getMessage());
        }
    }
    ...
    private void showFirms(){
        String firms = "";
        controller.EntityReader<domain.Firm> firmReader = this.applicationController.getAllFirms();
    ...
    private void showMenu(){
        for (FirmMenuOptions item : userInterface.FirmMenuOptions.values())
            this.output.println((item.ordinal() + 1) + ". " + item.name().replaceAll("_", " "));
    }

    // Reads an option (Integer). If an invalid number format is entered -1 will be
    // returned.
