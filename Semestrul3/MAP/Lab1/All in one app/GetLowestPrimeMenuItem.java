...
public class GetLowestPrimeMenuItem extends MenuItem {
    // Creates a new GetFirstPrimeMenuItem instance.
    // Preconditions:
    //      controller: a Controller that is used for business logic.
    public GetLowestPrimeMenuItem(Controller controller) {
        super("Get the lowest prime from an array");
        this.controller = controller;
    }

    @Override
    // Reads an array from the given input and prints to the given output the lowest prime number found.
    // Preconditions:
    ...
    public void run(java.io.BufferedReader input, java.io.PrintStream output) throws java.io.IOException {
        ...
        if (number != -1)
            output.println("The lowest prime number from the given array is " + number);
        else
