...
namespace FamilyExpenses.ViewModels
{
		...
		: INotifyPropertyChanged
	{
		private static class Cache
		{
			...
			internal static Income GetIncome(XElement incomeXElement, Person person)
			{
				return new Income(int.Parse(incomeXElement.Element("sum").Value),
								  XmlConvert.ToDateTime(incomeXElement.Element("dateReceived").Value,
														"yyyy-MM-dd\\THH:mm:ss.FFFFFFFzzz"),
								  person);
			}
			internal static Purchase GetPurchase(Person person, XElement purchaseElement, string xmlFileName)
			{
				if (person == null)
			...
			internal static IReadOnlyList<Shop> AllShops(string xmlFileName)
			{
				foreach (XElement addressElement in XDocument.Load(xmlFileName)
															 .Root
															 .Elements("Addresses"))
		...
		public MainViewModel()
		{
			...
			_addPurchaseCommand = new RelayCommand(parameter =>
			{
				...
				_LoadNodesInfo();
				_LoadPurchases();
				_OnPropertyChanged("PersonBalance");
			});
			...
			_exportToHtmlCommand = new RelayCommand(_GenerateHtmlWithPurchases, false);
			_exportToXmlCommand = new RelayCommand(_GenerateXmlWithPurchases, false);
			_addIncomeCommand = new RelayCommand((parameter) =>
			{
				Tuple<int, DateTime> incomeInfo = (Tuple<int, DateTime>)parameter;
				XDocument xDocument = XDocument.Load(_xmlFileName);
				xDocument.Element("FamilyExpenses")
						 .Elements("Persons")
						 .FirstOrDefault(personXElement => personXElement.Element("name").Value == _loggedinPerson.Name)
						 .Add(new XElement("Incomes",
										   new XElement("sum")
										   {
											   Value = incomeInfo.Item1.ToString()
										   },
										   new XElement("dateReceived")
										   {
											   Value = incomeInfo.Item2.ToString("yyyy-MM-dd\\THH:mm:ss.FFFFFFFzzz")
										   }));
				xDocument.Save(_xmlFileName);
				_OnPropertyChanged("Incomes");
				_OnPropertyChanged("PersonBalance");
			});
			_filterTreeNode.Children.Add(_monthsTreeNode);
			_filterTreeNode.Children.Add(_shopsTreeNode);
		...
		public string PersonBalance
		{
			get
			{
				...
				XQueryEvaluator totalIncomeEvaluator = processor.NewXQueryCompiler()
																.Compile("sum(for $person in /FamilyExpenses/Persons where string($person/name) = \""
																		 + _loggedinPerson.Name
																		 + "\" return $person/Incomes/sum)")
																.Load();
				XQueryEvaluator totalCostsEvaluator = processor.NewXQueryCompiler()
													  .Compile("sum(for $person in /FamilyExpenses/Persons where string($person/name) = \""
																	+ _loggedinPerson.Name
																	+ "\" return sum(for $purchase in $person/Purchases return ($purchase/price * $purchase/quantity)))")
													  .Load();
				using (XmlReader xmlFileStream = XmlReader.Create(_xmlFileName, _xmlSettings))
					totalIncomeEvaluator.ContextItem = processor.NewDocumentBuilder()
																.Build(xmlFileStream);
				using (XmlReader xmlFileStream = XmlReader.Create(_xmlFileName, _xmlSettings))
					totalCostsEvaluator.ContextItem = processor.NewDocumentBuilder()
															   .Build(xmlFileStream);
				IEnumerator totalIncomeResult = totalIncomeEvaluator.Evaluate().GetEnumerator();
				IEnumerator totalCostsResult = totalCostsEvaluator.Evaluate().GetEnumerator();
				totalIncomeResult.MoveNext();
				totalCostsResult.MoveNext();
				return ("Balance: " + (int.Parse(totalIncomeResult.Current.ToString()) - int.Parse(totalCostsResult.Current.ToString())) + " " + LoggedinPerson.PreferedCurrency.ToString());
			}
		}
		...
		public IReadOnlyList<Shop> AllShops
		{
			get
			{
				return Cache.AllShops(_xmlFileName);
			}
		}
		...
		public IEnumerable<Income> Incomes
		{
			get
			{
				return XDocument.Load(_xmlFileName)
								.Element("FamilyExpenses")
								.Elements("Persons")
								.First(personXElement => personXElement.Element("name").Value == _loggedinPerson.Name)
								.Elements("Incomes")
								.Select(incomeXElement => Cache.GetIncome(incomeXElement, _loggedinPerson));
			}
		}
		...
		private void _LoadPurchases()
		{
			if (_selectorTreeNode != null)
			{
				...
				{
					_exportToHtmlCommand.CanExecuteCommand = true;
					_exportToXmlCommand.CanExecuteCommand = true;
				}
				else
				{
					_exportToHtmlCommand.CanExecuteCommand = false;
					_exportToXmlCommand.CanExecuteCommand = false;
				}
			}
		}
		...
		private void _GenerateHtmlWithPurchases(object obj)
		{
...
<body>
	<table>
		...
		<tr>
			...
			<th>Shop</th>
			</tr>", _loggedinPerson.PreferedCurrency);
					foreach (Purchase purchase in _purchases.OrderBy(p => p.PurchaseDate))
						streamWriter.Write(@"
			<tr>
				<td>{0}</td>
				<td>{1}</td>
				...
				<td>{5}</td>
										   ...
										   purchase.Product.Producer.Name,
										   purchase.Price,
										   purchase.Quantity,
										   purchase.PurchaseDate.ToString("MMMM dd, yyyy"),
										   purchase.Shop.Name);
					streamWriter.Write(@"
	</table>
</body>
		...
		private void _GenerateXmlWithPurchases(object obj)
		{
			SaveFileDialog saveFileDialog = new SaveFileDialog()
			{
				AddExtension = true,
			...
			if (saveFileDialog.ShowDialog().GetValueOrDefault())
			{
				_WriteXsltFile(saveFileDialog.FileName.Substring(0, saveFileDialog.FileName.LastIndexOf('\\')));
				using (StreamWriter streamWriter = new StreamWriter(saveFileDialog.OpenFile()))
				{
					streamWriter.Write(@"<?xml version=""1.0"" encoding=""UTF-8""?>
<?xml-stylesheet type=""text/xsl"" href=""transform.xsl""?>
<purchases>
	<currency>{0}</currency>", _loggedinPerson.PreferedCurrency.ToString());
					foreach (Purchase purchase in _purchases.OrderBy(p => p.PurchaseDate))
						streamWriter.Write(@"
	<purchase>
		<product>{0}</product>
		...
		private void _WriteXsltFile(string path)
		{
			using (StreamWriter streamWriter = new StreamWriter(File.OpenWrite(path + "/transform.xsl")))
...
<xsl:template match=""/"">
	<html>
		...
		<body>
			<table>
				...
				<tr>
					<th>Product</th>
					<th>Producer</th>
					<th>Price (<xsl:value-of select=""/purchases/currency""/>)</th>
					<th>Quanity</th>
					<th>Date purchased</th>
					<th>Shop</th>
				</tr>
				...
				<tr>
					...
					<td><xsl:value-of select=""quanity""/></td>
		...
		private readonly RelayCommand _exportToHtmlCommand;
		private readonly RelayCommand _exportToXmlCommand;
